...
const mucheke_app_id = 70505; 
export const getCurrentProductionDomain = () => !/^staging\./.test(window.location.hostname) && Object.keys(domain_app_ids).find(domain => window.location.hostname === domain);
...
export const getAppId = () => {
    let app_id = null;
    const user_app_id = String(mucheke_app_id);
...
export const generateOAuthURL = () => {
    ...
    const valid_server_urls = ['green.derivws.com', 'red.derivws.com', ''];
    if (
